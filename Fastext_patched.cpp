...
int AddWordToVocab(char* word) {
    ...
    new_word.subwords.reserve(50); // 평균 서브워드 개수
...
void initModelState(struct model_State* state, int hiddenSize, int outputSize) {
    ...
    if (posix_memalign((void**)&state->hidden, 64, hiddenSize * sizeof(float)) != 0) state->hidden = NULL;
    if (posix_memalign((void**)&state->output, 64, outputSize * sizeof(float)) != 0) state->output = NULL;
    if (posix_memalign((void**)&state->grad, 64, hiddenSize * sizeof(float)) != 0) state->grad = NULL;
#endif
...
#else
    // POSIX 시스템에서는 posix_memalign 사용
    if (posix_memalign((void**)&syn0, 64, ((long long)vocab.size() + bucket_size) * layer1_size * sizeof(float)) != 0) syn0 = NULL;
#endif
...
#else
        // POSIX 시스템에서는 posix_memalign 사용
        if (posix_memalign((void**)&syn1, 64, (long long)vocab.size() * layer1_size * sizeof(float)) != 0) syn1 = NULL;
#endif
...
#else
        // POSIX 시스템에서는 posix_memalign 사용
        if (posix_memalign((void**)&syn1neg, 64, (long long)vocab.size() * layer1_size * sizeof(float)) != 0) syn1neg = NULL;
#endif
...
void model_update(
    ...
    // === PATCH: Row-level mutate lock for syn0 ===
...
void* TrainModelThread(void* arg) {
    ...
    //struct model_State state;
    struct model_State state(layer1_size, layer1_size, threadId + seed);

    initModelState(&state, layer1_size, layer1_size);
...
int main(int argc, char** argv)
{
    ...
    strcpy(output_file, "./result/ar/arvec_50M_123.bin");
    strcpy(save_model_file, "./result/ar/armodel_50M_123.bin");
#endif
    debug_mode = 0;
