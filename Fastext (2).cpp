// Fastext.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
struct model_State {
    ...
    std::minstd_rand rng;
        // 생성자 정의
    model_State() : lossValue_(0.0f), nexamples_(0), hidden(nullptr),
                    output(nullptr), grad(nullptr), rng(0) {}
};
...
void InitUnigramTable() {
    ...
    for (a = 0; a < vocab.size(); a++) train_words_pow += pow(vocab[a].cn, power);
    i = 0;
...
long long GetWordHash(char* word) {
    ...
    for (a = 0; a < strlen(word); a++) {
        ...
        //printf("jvjhv %lld", hash);
        hash = hash ^ (long long)(unsigned char)word[a];
...
int GetHash(char* word) {
    ...
    //for (a = 0; a < strlen(word); a++) hash = hash * 257 + (unsigned char)word[a];
...
int AddWordToVocab(char* word) {
    ...
    new_word.subwords.reserve(10); // 평균 서브워드 개수
    int new_index = vocab.size(); // push_back 전에 인덱스 계산
...
void CreateBinaryTree() {
    ...
    for (a = 0; a < vocab.size(); a++) {
        ...
        vocab[a].codelen = i;
        vocab[a].point[0] = vocab.size() - 2;
        for (b = 0; b < i; b++) {
            vocab[a].code[i - b - 1] = code[b];
            vocab[a].point[i - b] = point[b] - vocab.size();
        }
    }
    free(count);
    free(binary);
...
void initNgrams() {
    for (size_t i = 0; i < vocab.size(); i++) {
        ...
        vocab[i].subwords.push_back(i); // 자기 자신 id 추가
...
//if (!subwords.empty()) {
    //printf(" computeSubwords subword  0index %d\n", subwords[0]);
    //printf("computeSubwords subword token ");
   // for (int hre = 0; hre < 15; hre++) {
        //printf("%c", vocab[subwords[0]].word[hre]);
    //}
    //printf("\n");
//}
//subwords.clear();
//subwords.reserve(20); // n-gram 개수를 고려한 예상 크기
int word_idx = SearchVocab((char*)word);
if (word_idx >= 0) {
    subwords.push_back(word_idx);
}
    for (int i = 0; i < buflen; i++) {
        ...
        for (int n = 1; n <= maxn; n++) {
            ...
            if ((n >= 1 && n <= 6) && !(i == 0 || jj == buflen)) {
                int h = GetWordHash(ngram) % bucket_size;
                int idx = vocab.size() + h;
...
int get_line(FILE* fin, std::vector<int>& words) {
    ...
    while (token[0] != '\0') {
        ...
        // === [추가: 빈도 기반 다운샘플링 적용] ===
        if (sample > 0) {
            float ran = (sqrt(vocab[wid].cn / (sample * train_words)) + 1) *
    ...
    // 디버깅 코드: words와 labels 출력
    //printf("[DEBUG] words: ");
    int wsize = words.size();
    if (debug_mode > 1) printf("[DEBUG] line size %d \n", wsize);
    //for (int i = 0; i < wsize; i++) {
    //    if (words[i] >= 0 && words[i] < vocab.size())
    //       printf("%s ", vocab[words[i]].word);
    //   else
    //      printf("%d ", words[i]);
    //}
    return ntokens;
...
int get_line_sub(FILE* fin, std::vector<int>& words, std::vector<int>& labels) {
    ...
    // labels 벡터 초기화
    words.clear();
    labels.clear(); // labels를 std::vector로 사용
    ReadWord(token, fin);
    while (token[0] != '\0') {
        int wid = SearchVocab(token);
        ...
        if (type == 0) {
            ...
            // labels를 std::vector로 추가
            labels.push_back(wid - vocab.size()); // 라벨 벡터에 추가
        }
    ...
    //}
    //printf("\n[DEBUG] labels: ");
    int lsize = labels.size(); // 벡터 크기 사용
    for (int i = 0; i < lsize; i++) {
        int idx = labels[i] + vocab.size();
        //if (labels[i] >= 0 && idx < vocab.size())
        //    printf("%s ", vocab[idx].word);
        //else
        //    printf("%d ", labels[i]);
    }
    //printf("\n");
...
void add_subwords(std::vector<int>& line, const char* token, int wid) {
    // 스레드별 정적 버퍼 사용 - malloc/free 오버헤드 제거
    ...
    if (wid < 0) {
        if (strcmp(token, EOS) != 0) {
            ...
            if (total_len <= sizeof(concat_buf)) {
                ...
                // 버퍼 크기 초과 시 fallback: 원본 토큰만 추가
                if (debug_mode > 0) {
                    printf("[WARN] Token too long for buffer: %s\n", token);
                }
                // 기본 단어 ID만 추가하거나 다른 처리
            }
        }
    }
    else {
        // 기존 단어: 사전에 계산된 서브워드 사용
        if (maxn <= 0) {
            line.push_back(wid);
        }
        else {
...
void initModelState(struct model_State* state, int hiddenSize, int outputSize, int seed) {
    
    state->lossValue_ = 0.0f;
    state->nexamples_ = 0;
#ifdef _MSC_VER
    state->hidden = (float*)_aligned_malloc(hiddenSize * sizeof(float), 128);
    state->output = (float*)_aligned_malloc(outputSize * sizeof(float), 128);
    state->grad = (float*)_aligned_malloc(hiddenSize * sizeof(float), 128);
#else
    // POSIX 시스템에서는 posix_memalign 사용
    if (posix_memalign((void**)&state->hidden, 128, hiddenSize * sizeof(float)) != 0) state->hidden = NULL;
    if (posix_memalign((void**)&state->output, 128, outputSize * sizeof(float)) != 0) state->output = NULL;
    if (posix_memalign((void**)&state->grad, 128, hiddenSize * sizeof(float)) != 0) state->grad = NULL;
#endif
    if (state->hidden) memset(state->hidden, 0, hiddenSize * sizeof(float));
    if (state->output) memset(state->output, 0, outputSize * sizeof(float));
    if (state->grad) memset(state->grad, 0, hiddenSize * sizeof(float));
...
void InitNet() {
    ...
    //std::uniform_real_distribution<float> uniform_dist(-1.0f / layer1_size, 1.0f / layer1_size);
#ifdef _MSC_VER
    syn0 = (float*)_aligned_malloc(((long long)vocab.size() + bucket_size) * layer1_size * sizeof(float), 128);
#else
    // POSIX 시스템에서는 posix_memalign 사용
    if (posix_memalign((void**)&syn0, 128, ((long long)vocab.size() + bucket_size) * layer1_size * sizeof(float)) != 0) syn0 = NULL;
#endif
    if (syn0 == NULL) { printf("Memory allocation failed\n"); exit(1); }
    if (loss_name == 1) {
#ifdef _MSC_VER
        syn1 = (float*)_aligned_malloc((long long)vocab.size() * layer1_size * sizeof(float), 128);
#else
        // POSIX 시스템에서는 posix_memalign 사용
        if (posix_memalign((void**)&syn1, 128, (long long)vocab.size() * layer1_size * sizeof(float)) != 0) syn1 = NULL;
#endif
        if (syn1 == NULL) { printf("Memory allocation failed\n"); exit(1); }
        for (a = 0; a < vocab.size(); a++) for (b = 0; b < layer1_size; b++)
            syn1[a * layer1_size + b] = 0;
        CreateBinaryTree();
    }
    if (negative > 0) {
#ifdef _MSC_VER
        syn1neg = (float*)_aligned_malloc((long long)vocab.size() * layer1_size * sizeof(float), 128);
#else
        // POSIX 시스템에서는 posix_memalign 사용
        if (posix_memalign((void**)&syn1neg, 128, (long long)vocab.size() * layer1_size * sizeof(float)) != 0) syn1neg = NULL;
#endif
        if (syn1neg == NULL) { printf("Memory allocation failed\n"); exit(1); }
        for (a = 0; a < vocab.size(); a++) for (b = 0; b < layer1_size; b++)
            syn1neg[a * layer1_size + b] = 0;
    }
    float uni = 1.0f / static_cast<float>(layer1_size); // fastText: uniform(-a, a)
    ...
    for (a = 0; a < vocab.size(); a++) for (b = 0; b < layer1_size; b++) {
        ...
        syn0[a * layer1_size + b] = uniform_dist_w_in(rng);
...
void supervised(
    ...
    // 디버깅: labels 출력
    //printf("[DEBUG] labels (%d): ", labels_size);
    //for (int i = 0; i < labels_size; i++) {
    //    printf("%d ", labels[i]);
    //}
    //printf("\n");
    // 디버깅: line 출력
    //printf("[DEBUG] line (%d): ", line_size);
...
void model_update(
    ...
    //printf("model_update skipping update.\n");
    if (input.empty()) {
        if (debug_mode > 1)printf("Empty input, skipping update.\n");
        return;
    }
    if (!targets.empty() && targetIndex >= 0 && targetIndex < targets.size()) {
    ...
    for (int i = 0; i < layer1_size; i++) {
        hidden_norm += state->hidden[i] * state->hidden[i];
    }
    hidden_norm = sqrt(hidden_norm);
    if (debug_mode > 1) printf("  hidden_norm=%.6f\n", hidden_norm);

    //  Gradient 변화 모니터링
    float grad_norm_before = 0.0f;
    for (int i = 0; i < layer1_size; i++) {
        state->grad[i] = 0.0f;
        grad_norm_before += state->hidden[i] * state->hidden[i];
    }
    grad_norm_before = sqrt(grad_norm_before);
    ...
    for (int i = 0; i < layer1_size; i++) {
        grad_norm_after += state->grad[i] * state->grad[i];
    }
    grad_norm_after = sqrt(grad_norm_after);

    if (debug_mode > 1) printf("Update  loss=%.4f, grad_norm=%.4f->%.4f, alpha=%.6f\n", lossValue, grad_norm_before, grad_norm_after, alpha);
    //normalizeGradient_ = true;
    ...
    for (int i = 0; i < inputSize; i++) {
        ...
        for (int j = 0; j < layer1_size; j++) {
            ...
            weight_update_norm += fabs(state->grad[j] * alpha);
        }
    }
    if (debug_mode > 1) printf("Weight update norm: %.6f\n", weight_update_norm);

    // gradient를 0으로 초기화 (다음 스텝을 위해)
    for (int i = 0; i < layer1_size; i++) {
        state->grad[i] = 0.0f;
    }
}
void addVectorToRow(float* syn0, const float* grad, int row, float a) {
    for (int j = 0; j < layer1_size; j++) {
        syn0[row * layer1_size + j] += grad[j] * a;
    }
}
...
// 모델 상태 초기화 시 Norm 확인
void debug_vector_norms() {
    static int call_count = 0;
    if (call_count++ % 10000 != 0) return;
    ...
    for (long long i = 0; i < vocab.size(); i++) {
        float norm = 0.0f;
        for (int j = 0; j < layer1_size; j++) {
            norm += syn0[i * layer1_size + j] * syn0[i * layer1_size + j];
        }
        norm = sqrt(norm);
        avg_norm += norm;
    ...
    if (count > 0) {
        avg_norm /= count;
        if (debug_mode > 1) printf("Vector norms - Avg: %.4f, Min: %.4f, Max: %.4f\n",
            avg_norm, min_norm, max_norm);
    }
}
void real_time_monitoring(struct model_State* state, int threadId) {
    static std::vector<float> recent_losses;
    static int monitor_count = 0;

    if (monitor_count++ % 100 == 0) {
        float current_loss = getLoss(state);
        recent_losses.push_back(current_loss);
        ...
        //if (threadId == 0) {
            //printf("Thread %d - Recent avg loss: %.4f, Current: %.4f\n",
        //        threadId, avg_loss, current_loss);
        //}
...
void averageRowsToVector(float* hidden, const std::vector<int>& input) {
    int n = input.size();
    for (int j = 0; j < layer1_size; j++) hidden[j] = 0.0f;
    for (int i = 0; i < n; i++) {
        int row = input[i];
        for (int j = 0; j < layer1_size; j++) {
            hidden[j] += syn0[row * layer1_size + j];
        }
    }
    if (n > 0) {
        for (int j = 0; j < layer1_size; j++) {
...
float loss_forward_old(const int* targets, int targetIndex, struct model_State* state, float alpha, bool isUpdate) {
    float loss = 0;
    int c, d, l2;
    float f, g;
    ...
    if (loss_name == 0) { // ns
        for (d = 0; d < negative + 1; d++) {
            ...
            if (d == 0) {
                ...
                //state->rng = state->rng * (unsigned long long)25214903917 + 11;
                //target = table[(state->rng >> 16) % table_size];
                //if (target == 0) target = state->rng % (vocab.size() - 1) + 1;
    ...
    if (loss_name == 0) { /* negative sampling */
        ...
        for (d = 0; d < negative + 1; d++) {
            ...
            if (d == 0) {
                ...
                //if (target == 0) {
                //  state->rng = state->rng * (unsigned long long)25214903917 + 11;
                //  target = state->rng % (vocab.size() - 1) + 1;
                //}
                //if (target == word) {
                //  rng = state->rng * (unsigned long long)25214903917 + 11;
                //  target = rng % (vocab.size() - 1) + 1;
                //  if (target == word) continue;
                //}
                  //if (target == word) {
                    //state->rng = state->rng * (unsigned long long)25214903917 + 11;
...
void SaveVectors() {
    ...
    fprintf(fo, "%lld %lld\n", (long long)vocab.size(), layer1_size);
#else
    ...
    fprintf(fo, "%lld %lld\n", (long long)vocab.size(), layer1_size);
#endif
    float* vec = (float*)malloc(layer1_size * sizeof(float));
    if (!vec) {
    ...
    for (a = 0; a < vocab.size(); a++) {
        memset(vec, 0, layer1_size * sizeof(float));
        int subword_count = 0;
        for (size_t i = 0; i < vocab[a].subwords.size(); i++) {
            int subword_id = vocab[a].subwords[i];
            for (b = 0; b < layer1_size; b++) {
                vec[b] += syn0[subword_id * layer1_size + b];
            }
            subword_count++;
        }
        if (subword_count > 0) {
...
void* TrainModelThread(void* arg) {
...
#endif
    fseek(fi, file_size / num_threads * threadId, SEEK_SET);
    long long localTokenCount = 0;
    struct model_State state;
    initModelState(&state, layer1_size, layer1_size, threadId + seed);
    ...
    while (keepTraining(totaltokens, tokenCount_) == 1) {
        ...
        //float fake_progress = (float)(tokenCount_) / (iter * totaltokens);
...
void SaveModel(const char* filename) {
    ...
    fwrite(syn0, sizeof(float), (vocab.size() + bucket_size) * layer1_size, fo);

    // (필요시 syn1, syn1neg도 저장)
    if (loss_name == 1 && syn1) {
        fwrite(syn1, sizeof(float), vocab.size() * layer1_size, fo);
    }
    if (negative > 0 && syn1neg) {
        fwrite(syn1neg, sizeof(float), vocab.size() * layer1_size, fo);
    }
...
#ifdef _MSC_VER
    strcpy_s(train_file, sizeof(train_file), "arwiki.txt");
    strcpy_s(output_file, sizeof(output_file), "arvec.bin");
    strcpy_s(save_model_file, sizeof(save_model_file), "armodel.bin");
#else
    //strcpy(train_file, "./data/wikimedia/ar/wiki.ar.txt");
    strcpy(train_file, "./data/wikimedia/ar/wiki.ar.50M.txt");
    strcpy(output_file, "./result/ar/arvec_50M_4.bin");
    strcpy(save_model_file, "./result/ar/armodel_50M_2.bin");
#endif
    debug_mode = 0;
