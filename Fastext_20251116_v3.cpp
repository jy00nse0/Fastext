...
void computeSubwords(const char* word, std::vector<int>& subwords, std::vector<std::string>& ngrams);
...
void initNgrams() {
    for (size_t i = 0; i < vocab.size(); i++) {
        ...
        if (strcmp(vocab[i].word, "</s>") != 0) {   // word가 char*일 경우
            ...
            computeSubwords(word_buf, vocab[i].subwords, ngrams);
        }
    }
}
        ...
        if (strcmp(token, EOS) != 0) {
            ...
            if (total_len <= sizeof(concat_buf)) {
                ...
                if (concat_idx >= 0) {
                    ...
                    if (new_idx >= 0) {
                        std::vector<std::string> ngrams;
                        computeSubwords(concat, vocab[new_idx].subwords, ngrams);
