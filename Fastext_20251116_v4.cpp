...
void computeSubwords(const char* word, std::vector<int>& subwords, int minNgram = -1, int maxNgram = -1);
...
void initNgrams() {
    for (size_t i = 0; i < vocab.size(); i++) {
        ...
        if (strcmp(vocab[i].word, "</s>") != 0) {   // word가 char*일 경우
            ...
            computeSubwords(word_buf, vocab[i].subwords, -1, -1);
        }
    }
}
...
void computeSubwords(const char* word, std::vector<int>& subwords, int minNgram, int maxNgram) {
    if (minNgram < 0) minNgram = minn;
    if (maxNgram < 0) maxNgram = maxn;
    ...
    subwords.reserve(subwords.size() + buflen * (maxNgram - minNgram + 1));
    ...
    if (buflen != 0) {
        for (int i = 0; i < buflen; i++) {
            ...
            for (int n = minNgram; n <= maxNgram; n++) {
                ...
                ngram[ngram_len] = '\0';
                if ((n >= minNgram && n <= maxNgram) && !(i == 0 || jj == buflen)) {
        ...
        if (strcmp(token, EOS) != 0) {
            ...
            if (total_len <= sizeof(concat_buf)) {
                ...
                if (concat_idx >= 0) {
                    ...
                    if (new_idx >= 0) {
                        computeSubwords(concat, vocab[new_idx].subwords, -1, -1);
