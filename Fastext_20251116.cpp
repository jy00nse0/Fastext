...
void CreateBinaryTree() {
    ...
    for (a = 0; a < vocab.size(); a++) {
        ...
        if (!vocab[a].point) vocab[a].point = (int*)calloc(MAX_CODE_LENGTH, sizeof(int));
        if (!vocab[a].code) vocab[a].code = (char*)calloc(MAX_CODE_LENGTH, sizeof(char));
        while (1) {
            code[i] = binary[b];
            point[i] = b;
            i++;
...
void LearnVocabFromTrainFile() {
    ...
    //auto new_end = std::remove_if(vocab.begin(), vocab.end(), [](const vocab_word& w) { return w.cn < min_count; });
...
int get_line_sub(FILE* fin, std::vector<int>& words, std::vector<int>& labels) {
    ...
    while (token[0] != '\0') {
        ...
        if (type == 0) {
            ...
            labels.push_back(wid - vocab.size());
        }
...
void averageRowsToVector(float* hidden, const std::vector<int>& input) {
    int n = input.size();
    if (n == 0) return;
    memset(hidden, 0, layer1_size * sizeof(float));
    for (int i = 0; i < n; i++) {
        int row = input[i];
        float* src = &syn0[row * layer1_size];
        float* dst = hidden;
