...
void computeSubwords(const std::string& word, std::vector<int>& subwords);
...
void initNgrams() {
    for (size_t i = 0; i < vocab.size(); i++) {
        ...
        if (strcmp(vocab[i].word, "</s>") != 0) {   // word가 char*일 경우
            ...
            std::string word_str(word_buf);
            computeSubwords(word_str, vocab[i].subwords);
        }
    }
}
...
void computeSubwords(const std::string& word, std::vector<int>& subwords) {
    const char* buf = word.c_str();
    int buflen = (int)word.length();
    if (buflen != 0) {
            ...
            if (total_len <= sizeof(concat_buf)) {
                ...
                if (concat_idx >= 0) {
                    ...
                    if (new_idx >= 0) {
                        ...
                        computeSubwords(concat_str, vocab[new_idx].subwords);
